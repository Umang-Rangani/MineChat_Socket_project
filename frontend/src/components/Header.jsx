...
export default function Header() {
  ...
  const isStatusActive = location.pathname === '/status'
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-[#2a3942] bg-[#202c33] px-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link to={'/'} className="text-xl font-semibold text-[#e9edef]">
          MineChat
        </Link>
      </div>
      ...
      <div className="flex items-center gap-1">
        ...
        {/* Chats */}
        <button onClick={() => navigate('/')} type="button" className={`flex items-center gap-2 rounded-lg px-4 py-2 transition ${isChatActive ? 'bg-[#2a3942] text-white' : 'text-[#aebac1] hover:bg-[#2a3942] hover:text-white'}`}>
        ...
        {/* Status */}
        <Link to="/status" className={`flex items-center gap-2 rounded-lg px-4 py-2 transition ${isStatusActive ? 'bg-[#2a3942] text-white' : 'text-[#aebac1] hover:bg-[#2a3942] hover:text-white'}`}>
        ...
        <div onClick={userHandle} className={`rounded-md p-1 text-[#aebac1] transition hover:text-white  ${isProfileActive ? 'bg-[#2a3942] text-white' : 'text-[#aebac1] hover:bg-[#2a3942] hover:text-white'}`}>
          {user && (
            <div className="flex items-center gap-3">
              {/* Profile Image */}
              {!user.image ? (
        ...
        <button title='logout' onClick={logout} type="button" className="flex items-center gap-2 rounded-lg px-3 py-2 text-[#aebac1] transition hover:bg-red-500/10 hover:text-red-400">
          <LogOut size={20} />
